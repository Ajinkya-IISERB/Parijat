...
if (file.exists("NO_PRL.gene.TMM.EXPR.matrix.RData") && ! NO_REUSE) {
    print('RESTORING DATA FROM EARLIER ANALYSIS')
    load("NO_PRL.gene.TMM.EXPR.matrix.RData")
} else {
    print('Reading matrix file.')
    primary_data = read.table("NO_PRL.gene.TMM.EXPR.matrix", header=T, com='', row.names=1, check.names=F, sep='\t')
    primary_data = as.matrix(primary_data)
}
source("/media/morpheus/disk2/NA/parijatk_rnaseq/trinityrnaseq-v2.15.1/Analysis/DifferentialExpression/R/heatmap.3.R")
source("/media/morpheus/disk2/NA/parijatk_rnaseq/trinityrnaseq-v2.15.1/Analysis/DifferentialExpression/R/misc_rnaseq_funcs.R")
source("/media/morpheus/disk2/NA/parijatk_rnaseq/trinityrnaseq-v2.15.1/Analysis/DifferentialExpression/R/pairs3.R")
source("/media/morpheus/disk2/NA/parijatk_rnaseq/trinityrnaseq-v2.15.1/Analysis/DifferentialExpression/R/vioplot2.R")
data = primary_data
myheatcol = colorpanel(75, 'purple','black','yellow')
samples_data = read.table("no_prl_sample.txt", header=F, check.names=F, fill=T)
...
write.table(data, file="NO_PRL.gene.TMM.EXPR.matrix.minRow10.CPM.log2.dat", quote=F, sep='	');
pdf("NO_PRL.gene.TMM.EXPR.matrix.minRow10.CPM.log2.prcomp.principal_components.pdf")
prin_comp_data = data
...
write.table(pca$rotation, file="NO_PRL.gene.TMM.EXPR.matrix.minRow10.CPM.log2.PCA.prcomp.scores", quote=F, sep="	")
write.table(pca$x, file="NO_PRL.gene.TMM.EXPR.matrix.minRow10.CPM.log2.PCA.prcomp.loadings", quote=F, sep="	")
PCA.loadings=pca$x
